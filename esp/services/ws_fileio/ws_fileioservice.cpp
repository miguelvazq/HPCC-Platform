...
void readTableContentXML(IEspSaveTableToFileRequest& req, StringBuffer& buf)
{
    buf.append("<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?><?xml-stylesheet href=\"../esp/xslt/xmlformatter.xsl\" type=\"text/xsl\"?><Table><Rows>");
    ...
    ForEachItemIn(ri, rows)
    {
        ...
        ForEachItemIn(ci, cells)
        {
            ...
            if (header && *header && (cellCount != headerCount))
                buf.append("<Cell>").append(cells.item(ci)).append("</Cell>");
            else
            {
...
void readTableContentJSON(IEspSaveTableToFileRequest& req, StringBuffer& buf)
{
}
...
void readTableContent(IEspSaveTableToFileRequest& req, CSaveTableToFileContentFormats contentFormat, MemoryBuffer& buf)
{
    ...
    if (contentFormat == CSaveTableToFileContentFormats_XML)
        readTableContentXML(req, content);
    else if (contentFormat == CSaveTableToFileContentFormats_JSON)
        readTableContentJSON(req, content);
    else //CSV
