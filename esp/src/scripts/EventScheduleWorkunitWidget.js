...
define([
    ...
    return declare("EventScheduleWorkunitWidget", [_TabContainerWidget], {
        ...
        postCreate: function (args) {
            ...
            this.pushEventForm = registry.byId(this.id + "PushEventForm");
        },
...
*/
        ...
        _onEventApply :function (event){
            ...
            WsWorkunits.WUPushEvent({
                PushEventText: retVal.EventName,
                EventText : retVal.EventText
            }).then(function (response) {

            });

             if (this.pushEventForm.validate()) {
                var context = this;
                var request = domForm.toObject(this.pushEventForm.id);
                WsWorkunits.WUPushEvent({
                    request: request
                }).then(function (response) {
                var firstTab = null;
                var tab = this.ensurePane(this.pushEventForm.id, request);
                if (firstTab) {
