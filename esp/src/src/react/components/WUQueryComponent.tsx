...
export const WUQueryComponent: React.FunctionComponent<WUQueryComponent> = () => {
    return <div style={{ maxWidth: "1024px" }}>
        <MaterialTable
            ...
            data={query => {
                if (history.location && history.location.state && query.page === 0 && query.orderBy === undefined && query.orderDirection !== "desc") {
                    // request = history.location.state as any;
                } else {
                    // history.replace("/workunits", query);
                }
                return wuService.WUQuery({
                    Sortby: query.orderBy?.field === "TotalClusterTime" ? "ClusterTime" : query.orderBy?.field,
