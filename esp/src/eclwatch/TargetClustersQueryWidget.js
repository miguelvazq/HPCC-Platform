    ...
    return declare("TargetClustersQueryWidget", [GridDetailsWidget, ESPUtil.FormHelper], {
        ...
        refreshGrid: function () {
            ...
            WsTopology.TpTargetClusterQuery({
                ...
                var newRows = [];
                var parents = [
                    {Name: "Target Clusters", type: "cluster", results: []}, //ESPPreflight.getTargetClusters()
                    // {Name: "Cluster Processes", type: "cluster", results: []},
                    // {Name: "System Servers", type: "cluster", results: []}
                ]

                if (lang.exists("TpTargetClusterQueryResponse.TpTargetClusters.TpTargetCluster", response)) {
                    results = response.TpTargetClusterQueryResponse.TpTargetClusters.TpTargetCluster;
                ...
                arrayUtil.forEach(parents, function (row, idx) {
                    ...
                    if (parents[0].results) {
                        arrayUtil.forEach(parents[0].results, function (clusters, clusterIdx) {
                            newRows.push({
                                __hpcc_parentName: "TargetClusters",
